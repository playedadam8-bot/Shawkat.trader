...
def execute_with_fallbacks(api_key: str, messages: list, max_tokens: int = 300):
    if not api_key or not api_key.strip():
        raise Exception("Secret Key is missing. Please enter it in the sidebar.")
...
def generate_text_signal(api_key: str, asset: str, timer: str, payout: int):
    ...
    prompt = f"""
    You are an elite master binary options algorithmic trader. Current local time is {current_local_time}.
    ...
    - Provide exact keys: "asset", "live_price", "signal", "execution_time", "accuracy", "reason", "mtg_advice".
    - "signal" must be strictly either "CALL" or "PUT".
    - "execution_time" must be dynamically calculated by you based on the current market momentum and candle structure, set to an exact future timestamp strictly UNDER 60 SECONDS from the current time ({current_local_time}) in HH:MM:SS format.
    
    Example format:
    {{"asset": "{asset}", "live_price": "1.08520", "signal": "CALL", "execution_time": "HH:MM:SS", "accuracy": "95.2%", "reason": "Analyzed technical market structure.", "mtg_advice": "No MTG needed"}}
    """

    messages = [{"role": "user", "content": prompt}]
    return execute_with_fallbacks(api_key, messages, max_tokens=250)
...
def generate_vision_signal(api_key: str, image_bytes: bytes):
    ...
    prompt = f"""
    You are an elite master binary options algorithmic trader. Current local time is {current_local_time}.
    Deeply analyze this Quotex chart screenshot. 
    Completely and independently determine the optimal trade expiry duration based entirely on your chart technical analysis (such as candle structure, volatility, and momentum).

    CRITICAL INSTRUCTIONS:
    - Output ONLY valid raw JSON. Do NOT include markdown code blocks.
    - Provide exact keys: "asset", "live_price", "signal", "execution_time", "expiry", "accuracy", "wick_and_candle_analysis", "mtg_advice", "reason".
    - "signal" must be strictly either "CALL" or "PUT".
    - "execution_time" must be dynamically calculated by you based on the chart's current candle completion timing, set to an exact future timestamp strictly UNDER 60 SECONDS from the current time ({current_local_time}) in HH:MM:SS format.
    - "expiry" must be completely chosen and calculated by you based on the visual price action without restriction.
    """
    ...
    return execute_with_fallbacks(api_key, messages, max_tokens=350)
...
if not api_key:
    st.warning("⚠️ Please enter your Secret Key in the sidebar to activate the web app.")
    st.stop()
...
with tab2:
    ...
    st.caption("Upload a chart screenshot to let the AI fully analyze momentum, wicks, and independently determine the optimal expiry.")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Analyze Chart & Generate Signal", type="primary"):
            with st.spinner("AI is determining optimal expiry and direction from chart..."):
                try:
                    img_bytes = uploaded_file.getvalue()
                    sig = generate_vision_signal(api_key, img_bytes)
                    st.session_state.last_signal = sig
                    # Dynamically read the expiry calculated by the AI
                    st.session_state.last_signal_timer = sig.get("expiry", "Dynamic AI Expiry")
...
if st.session_state.last_signal:
    ...
    with res_col1:
        ...
        # Displays the AI-determined expiry directly from JSON output if available
        st.write(f"**Expiry Duration:** `{sig_data.get('expiry', st.session_state.get('last_signal_timer', '1 Minute'))}`")
